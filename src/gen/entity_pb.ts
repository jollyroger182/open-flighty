// @generated by protoc-gen-es v2.12.0 with parameter "target=ts"
// @generated from file entity.proto (syntax proto3)
/* eslint-disable */

import type { GenFile, GenMessage } from "@bufbuild/protobuf/codegenv2";
import { fileDesc, messageDesc } from "@bufbuild/protobuf/codegenv2";
import type { Airport } from "./entities/airport_pb";
import { file_entities_airport } from "./entities/airport_pb";
import type { Airline } from "./entities/airline_pb";
import { file_entities_airline } from "./entities/airline_pb";
import type { Sharing } from "./entities/sharing_pb";
import { file_entities_sharing } from "./entities/sharing_pb";
import type { Connection } from "./entities/connection_pb";
import { file_entities_connection } from "./entities/connection_pb";
import type { Profile } from "./entities/profile_pb";
import { file_entities_profile } from "./entities/profile_pb";
import type { Ticket } from "./entities/ticket_pb";
import { file_entities_ticket } from "./entities/ticket_pb";
import type { Flight } from "./entities/flight_pb";
import { file_entities_flight } from "./entities/flight_pb";
import type { City } from "./entities/city_pb";
import { file_entities_city } from "./entities/city_pb";
import type { Model } from "./entities/model_pb";
import { file_entities_model } from "./entities/model_pb";
import type { Message } from "@bufbuild/protobuf";

/**
 * Describes the file entity.proto.
 */
export const file_entity: GenFile = /*@__PURE__*/
  fileDesc("", [file_entities_airport, file_entities_airline, file_entities_sharing, file_entities_connection, file_entities_profile, file_entities_ticket, file_entities_flight, file_entities_city, file_entities_model]);

/**
 * @generated from message Entity
 */
export type Entity = Message<"Entity"> & {
  /**
   * @generated from field: Airport airport = 1;
   */
  airport?: Airport | undefined;

  /**
   * @generated from field: Airline airline = 2;
   */
  airline?: Airline | undefined;

  /**
   * @generated from field: Sharing sharing = 4;
   */
  sharing?: Sharing | undefined;

  /**
   * @generated from field: Connection connection = 5;
   */
  connection?: Connection | undefined;

  /**
   * @generated from field: Profile profile = 8;
   */
  profile?: Profile | undefined;

  /**
   * @generated from field: Ticket ticket = 12;
   */
  ticket?: Ticket | undefined;

  /**
   * @generated from field: Flight flight = 15;
   */
  flight?: Flight | undefined;

  /**
   * @generated from field: City city = 19;
   */
  city?: City | undefined;

  /**
   * @generated from field: Model model = 22;
   */
  model?: Model | undefined;
};

/**
 * Describes the message Entity.
 * Use `create(EntitySchema)` to create a new message.
 */
export const EntitySchema: GenMessage<Entity> = /*@__PURE__*/
  messageDesc(file_entity, 0);

